...
class WebService: NSObject {
    ...
    func applyForJob(jobId: Int, complete:@escaping ([String:AnyObject], Bool) -> ()) ->() {
        let url = baseUrl + "workers/jobs/\(jobId)/apply"
        SVProgressHUD.show()
    ...
    func getWorkerJobs(complete:@escaping ([String:AnyObject], Bool) -> ()) ->() {
        let url = baseUrl + "workers/jobs"
        SVProgressHUD.show()
        self.startRequest(.get, urlStr: url) { (response, isNull) in
            complete(response, isNull)
        }
    ...
    func getJobsByStatus(status: String, complete:@escaping ([String:AnyObject], Bool) -> ()) ->() {
        let url = baseUrl + "workers/jobs/applies?status=" + status
        SVProgressHUD.show()
        self.startRequest(.get, urlStr: url) { (response, isNull) in
            complete(response, isNull)
        }
